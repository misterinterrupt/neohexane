...
public:
  ...
  virtual void addSoundfile(const char *label, const char *url, Soundfile **zone)
  {
    ...
    fUI << "\"url\": \"" << url << "\""
        << ",";
    tab(fTab, fUI);
    fUI << "\"address\": \"" << path << "\"" << ((fPathTable.size() > 0) ? "," : "");
    if (fPathTable.size() > 0)
    {
      tab(fTab, fUI);
      fUI << "\"index\": \"" << getAddressIndex(path) << "\"";
  ...
  virtual void declare(const char *key, const char *value)
  {
    ...
    if ((strcmp(key, "name") == 0) && (fName == ""))
      fName = value;
    // fFileName found in metadata
    if ((strcmp(key, "filename") == 0) && (fFileName == ""))
      fFileName = value;
    tab(fTab, fMeta);
    fMeta << "{ "
          << "\"" << key << "\""
  ...
  std::string JSON(bool flat = false)
  {
    ...
    JSON << "\"name\": \"" << fName << "\",";
    tab(fTab, JSON);
    JSON << "\"filename\": \"" << fFileName << "\",";
    if (fVersion != "")
    {
      tab(fTab, JSON);
      JSON << "\"version\": \"" << fVersion << "\",";
    }
    if (fCompileOptions != "")
    {
      tab(fTab, JSON);
      JSON << "\"compile_options\": \"" << fCompileOptions << "\",";
    }
    ...
    {
      tab(fTab, JSON);
      JSON << "\"sha_key\": \"" << fSHAKey << "\",";
    }
    if (fExpandedCode != "")
    {
      tab(fTab, JSON);
      JSON << "\"code\": \"" << fExpandedCode << "\",";
    }
    tab(fTab, JSON);
    JSON << "\"inputs\": \"" << fInputs << "\",";
    tab(fTab, JSON);
    JSON << "\"outputs\": \"" << fOutputs << "\",";
    if (fSRIndex != -1)
    {
      tab(fTab, JSON);
      JSON << "\"sr_index\": \"" << fSRIndex << "\",";
    }
    tab(fTab, fMeta);
    fMeta << "],";
    tab(fTab, fUI);
    ...
    fTab -= 1;
    if (fCloseMetaPar == ',')
    { // If "declare" has been called, fCloseMetaPar state is now ','
...
public:
  JSONUI(const std::string &name,
         ...
         const std::vector<std::string> &include_pathnames,
         const std::string &size,
         const std::map<std::string, int> &path_table) : JSONUIAux<FAUSTFLOAT>(name, filename,
                                                                               inputs, outputs,
                                                                               sr_index,
                                                                               sha_key, dsp_code,
  ...
  JSONUI(const std::string &name, const std::string &filename, int inputs, int outputs) : JSONUIAux<FAUSTFLOAT>(name, filename, inputs, outputs)
  {
  }
...
public:
  ...
  {
    if (midi_dsp)
      fMidiInputs.push_back(midi_dsp);
  }
...
public:
  Interpolator3pt(double lo, double mi, double hi, double v1, double vm, double v2) : fSegment1(lo, mi, v1, vm),
...
public:
  LinearValueConverter(double umin, double umax, double fmin, double fmax) : fUI2F(umin, umax, fmin, fmax), fF2UI(fmin, fmax, umin, umax)
  {
  }
...
public:
  LogValueConverter(double umin, double umax, double fmin, double fmax) : LinearValueConverter(umin, umax, log(std::max<double>(DBL_MIN, fmin)), std::log(std::max<double>(DBL_MIN, fmax)))
  {
  }
...
public:
  ExpValueConverter(double umin, double umax, double fmin, double fmax) : LinearValueConverter(umin, umax, exp(fmin), exp(fmax))
  {
  }
...
public:
  AccUpConverter(double amin, double amid, double amax, double fmin, double fmid, double fmax) : fA2F(amin, amid, amax, fmin, fmid, fmax),
...
public:
  AccDownConverter(double amin, double amid, double amax, double fmin, double fmid, double fmax) : fA2F(amin, amid, amax, fmax, fmid, fmin),
...
public:
  AccUpDownConverter(double amin, double amid, double amax, double fmin, double fmid, double fmax) : fA2F(amin, amid, amax, fmin, fmax, fmin),
...
public:
  AccDownUpConverter(double amin, double amid, double amax, double fmin, double fmid, double fmax) : fA2F(amin, amid, amax, fmax, fmin, fmax),
                                                                                                     fF2A(fmin, fmax, amin, amax) // Special, pseudo inverse of a non monotone function
  {
  }
...
public:
  ...
  virtual void buildUserInterface(UI *ui_interface)
  {
    ...
    ui_interface->addHorizontalSlider("carrier freq", &fHslider3, 90.0f, 20.0f, 15000.0f, 10.0f);
    ui_interface->addHorizontalSlider("carrier gain", &fHslider2, 0.5f, 0.0f, 1.0f, 0.00999999978f);
    ui_interface->addHorizontalSlider("modulator freq", &fHslider1, 110.0f, 20.0f, 15000.0f, 10.0f);
    ui_interface->addHorizontalSlider("modulator gain", &fHslider0, 0.5f, 0.0f, 1.0f, 0.00999999978f);
    ui_interface->addHorizontalSlider("modulator index", &fHslider4, 10000.0f, 0.0f, 15000.0f, 10.0f);
  ...
  virtual void compute(int count, FAUSTFLOAT **inputs, FAUSTFLOAT **outputs)
  {
    ...
    for (int i = 0; (i < count); i = (i + 1))
    {
      ...
      float fTemp0 = ftbl0mydspSIG0[int((65536.0f * fRec2[0]))];
      output0[i] = FAUSTFLOAT((fRec0[0] * fTemp0));
      fRec3[0] = (fSlow2 + (0.999000013f * fRec3[1]));
      fRec5[0] = (fSlow4 + (0.999000013f * fRec5[1]));
      float fTemp1 = (fRec4[1] + (fConst0 * (fSlow3 + (fRec5[0] * fTemp0))));
      fRec4[0] = (fTemp1 - std::floor(fTemp1));
      output1[i] = FAUSTFLOAT((fRec3[0] * ftbl0mydspSIG0[int((65536.0f * fRec4[0]))]));
      fRec0[1] = fRec0[0];
      fRec2[1] = fRec2[0];
      fRec3[1] = fRec3[0];
      fRec5[1] = fRec5[0];
...
exfaust0::exfaust0() : AudioStream((fDSP = new mydsp())->getNumInputs(), new audio_block_t *[fDSP->getNumInputs()])
{
  fUI = new MapUI();
...
void exfaust0::setParamValue(const std::string &path, float value)
{
